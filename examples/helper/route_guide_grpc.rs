...
impl RouteGuideClient {
    pub fn new(channel: Channel) -> RouteGuideClient {
        RouteGuideClient { client: Client::new(channel) }
    }
    ...
    pub fn get_feature_opt(&self, point: Point, opt: CallOption) -> Result<Feature> {
        self.client
            .unary_call(&METHOD_ROUTE_GUIDE_GET_FEATURE, point, opt)
    }
    ...
    pub fn get_feature_async_opt(&self,
                                 point: Point,
                                 opt: CallOption)
                                 -> Result<UnaryCallHandler<Feature>> {
        self.client
            .unary_call_async(&METHOD_ROUTE_GUIDE_GET_FEATURE, point, opt)
    }
    ...
    pub fn list_features_opt(&self,
                             rect: Rectangle,
                             opt: CallOption)
                             -> Result<ServerStreamingCallHandler<Feature>> {
        self.client
            .server_streaming(&METHOD_ROUTE_GUIDE_LIST_FEATURES, rect, opt)
    }
    ...
    pub fn record_route_opt(&self,
                            opt: CallOption)
                            -> Result<ClientStreamingCallHandler<Point, RouteSummary>> {
        self.client
    ...
    pub fn route_chat_opt(&self,
                          opt: CallOption)
                          -> Result<DuplexStreamingCallHandler<RouteNote, RouteNote>> {
        self.client
...
pub trait RouteGuide {
    ...
    fn record_route(&self,
                    ctx: RpcContext,
                    point: RequestStream<Point>,
                    resp: ClientStreamingResponseSink<RouteSummary>);
    fn route_chat(&self,
                  ctx: RpcContext,
                  note: RequestStream<RouteNote>,
                  resp: ResponseSink<RouteNote>);
}
...
pub fn create_service<R: RouteGuide + Send + Clone + 'static>(service: R) -> Service {
    ...
    let instance = service.clone();
    builder = builder.add_server_streaming_handler(&METHOD_ROUTE_GUIDE_LIST_FEATURES,
                                                   move |ctx, point, resp| {
                                                       instance.list_features(ctx, point, resp)
                                                   });
    let instance = service.clone();
    builder = builder.add_client_streaming_handler(&METHOD_ROUTE_GUIDE_RECORD_ROUTE,
                                                   move |ctx, point, resp| {
                                                       instance.record_route(ctx, point, resp)
                                                   });
    builder
        .add_duplex_streaming_handler(&METHOD_ROUTE_GUIDE_ROUTE_CHAT,
                                      move |ctx, point, resp| service.route_chat(ctx, point, resp))
        .build()
}
