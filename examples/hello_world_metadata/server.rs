...
impl Greeter for GreeterService {
    fn say_hello(&self, ctx: RpcContext, req: HelloRequest, sink: UnarySink<HelloReply>) {
        for index in 0..ctx.metadata().count() {
            let key = std::str::from_utf8(ctx.metadata().key(index)).expect("key");
            let value = std::str::from_utf8(ctx.metadata().value(index)).expect("value");
            info!("metadata entry {}:  {} -> {} ", index, key, value);
        }
