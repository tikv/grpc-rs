...
fn read_single_crt(name: &str) -> Result<String, io::Error> {
    let mut crt = String::new();
    fs::File::open(format!("tests/certs/{}.crt", name))
        .unwrap()
...
fn read_cert_pair(name: &str) -> Result<(String, String), io::Error> {
    let mut crt = String::new();
    ...
    fs::File::open(format!("tests/certs/{}.crt", name))
        ...
        .read_to_string(&mut crt)
        .unwrap();
    fs::File::open(format!("tests/certs/{}.key", name))
        .unwrap()
