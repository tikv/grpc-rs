...
impl RouteGuide for RouteGuideService {
    fn get_feature(&mut self, ctx: RpcContext<'_>, point: Point, sink: UnarySink<Feature>) {
        ...
        let resp = data
            .iter()
            .find(|f|same_point(f.location.0.as_ref().unwrap(), &point))
    ...
    fn list_features(
        ...
        let features: Vec<_> = data
            .iter()
            .filter_map(move |f| {
                if fit_in(f.location.0.as_ref().unwrap(), &rect) {
        ...
        let f = async move {
            ...
            while let Some(point) = points.try_next().await? {
                ...
                let valid_point = data.iter().any(|f| {
                    !f.name.is_empty() && same_point(&f.location.0.as_ref().unwrap(), &point)
                });
                if valid_point {
