...
impl WorkerService for Worker {
        ...
        self.remote.spawn(move |_| {
            ...
            sink.sink_map_err(Error::from).send_all(stream.map_err(Error::from).and_then(move |arg| {
                if arg.has_setup() {
                    ...
                    let s = try!(Server::new(env.clone(), cfg, remote.clone()));
        ...
        self.remote.spawn(move |_| {
            ...
            sink.sink_map_err(Error::from).send_all(stream.map_err(Error::from).and_then(move |arg| {
                if arg.has_setup() {
                    ...
                    client.take();
                    let c = Client::new(env.clone(), cfg, remote.clone());
                    client = Some(c);
                    Ok(ClientStatus::new())
                } else {
                    let mark = arg.get_mark();
    ...
    fn core_count(&self, _: RpcContext, _: CoreRequest, sink: UnaryResponseSink<CoreResponse>) {
        ...
        self.remote.spawn(|_| sink.success(resp).map_err(|e| println!("failed to report cpu count: {:?}", e)))
    }
    ...
    fn quit_worker(&self, _: RpcContext, _: Void, sink: ::grpc::UnaryResponseSink<Void>) {
        ...
        self.remote.spawn(|_| sink.success(Void::new()).map_err(|e| println!("failed to report quick worker: {:?}", e)));
